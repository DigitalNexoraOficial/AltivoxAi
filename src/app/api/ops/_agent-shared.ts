/**
 * Shared helpers for /api/ops/agents* and /api/ops/agent-runs*
 */
...
import { AgentError } from "@/core/agent-runtime";
...
export async function handleAgentError(
  ...
  if (err instanceof AgentError) {
    const result =
      err.code === "forbidden"
        ? "deny"
        : err.code === "persistence_error" || err.code === "execution_error"
          ? "error"
          : "rejected";
    await writeAuditEvent({
...
export async function readJson(
  ...
  try {
    ...
    if (!body || typeof body !== "object" || Array.isArray(body)) {
      throw new AgentError("invalid_input", "invalid_json_body");
    }
    return body as Record<string, unknown>;
  } catch (e) {
    if (e instanceof AgentError) throw e;
    throw new AgentError("invalid_input", "invalid_json");
  }
}
