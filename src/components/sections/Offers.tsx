...
export function Offers() {
  ...
  return (
    <section id="ofertas" className="section-pad relative mesh-divider">
      <div className="mx-auto max-w-7xl">
        <Reveal className="text-center">
          <span className="eyebrow">{t.offers.eyebrow}</span>
          <h2 className="heading-display mt-5 text-3xl md:text-5xl">{t.offers.title}</h2>
          <p className="mx-auto mt-4 max-w-2xl text-sm text-mist">{t.offers.sub}</p>
        </Reveal>

        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {t.offers.items.map((offer, i) => (
            <Reveal key={offer.title} delay={i * 0.08}>
              <article className={`glass ui-lift relative flex h-full flex-col rounded-[1.75rem] p-8 ${i === 1 ? "border-cyan/40 shadow-glow lg:-translate-y-3" : ""}`}>
                {i === 1 ? (
                ...
                <a href="#contact" className="btn-ghost ui-lift mt-8 w-full">
                  {offer.cta}
                </a>
              </article>
            </Reveal>
