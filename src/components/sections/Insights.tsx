          ...
          {t.blog.items.map((b, i) => (
            <Reveal key={b.title} delay={i * 0.06}>
              <button
                type="button"
                onClick={() => setOpen(i)}
                className="glass h-full w-full rounded-[1.75rem] p-6 text-left transition hover:-translate-y-1 hover:border-cyan/30"
              >
                <p className="font-mono text-[10px] uppercase tracking-widest text-cyan">
                  {b.meta}
                </p>
                <h3 className="mt-4 font-display text-sm uppercase text-white">
                  {b.title}
                </h3>
      ...
      {item ? (
        ...
        >
          ...
          >
            ...
            <button type="button" className="btn-ghost mt-6" onClick={() => setOpen(null)}>
              OK
            </button>
          </div>
        </div>
