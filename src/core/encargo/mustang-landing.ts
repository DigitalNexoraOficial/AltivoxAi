/**
 * Mustang landing optimized for ~60 FPS scroll scrub.
 * Lighter GLB + rAF-driven progress + minimal GPU cost (no shadows/AA).
 */
...
function modelUrlCandidates(file: string): string[] {
  const pin = "cursor/mustang-60fps-4521";
...
scene.add(rim);
const engineLight = new THREE.PointLight(0xffe0b0, 0, 4.5, 2);
engineLight.position.set(0, 1.35, 1.15);
...
function isHoodObject(o) {
  let p = o;
  while (p) {
    if (/Kapoot|Rooye_Kapoot|Rooye Kapoot|HoodPivot/i.test(p.name || "")) return true;
    p = p.parent;
  }
  return false;
}

/** Merge static (non-hood) meshes by material → far fewer draw calls. */
function mergeStaticMeshes(root) {
  ...
  root.traverse((o) => {
    if (!o.isMesh || isHoodObject(o)) return;
    const mats = Array.isArray(o.material) ? o.material : [o.material];
    if (mats.length !== 1 || !mats[0]) return;
    const mat = toStandard(mats[0]);
    const key = (mat.map ? mat.map.uuid : "nomap") + "|" + (mat.transparent ? "t" : "o") + "|" + mat.color.getHexString() + "|" + (mat.metalness ?? 0).toFixed(2);
    if (!groups.has(key)) groups.set(key, { mat, geos: [] });
    const g = o.geometry.clone();
    bake.multiplyMatrices(invRoot, o.matrixWorld);
    g.applyMatrix4(bake);
...
try {
  ...
  fitCar(gltf.scene);
  car.add(gltf.scene);
  rigAnimatableParts(car);      // keep Kapoot hierarchy
  cheapifyMaterials(car);
  mergeStaticMeshes(car);       // batch non-hood into few draw calls (car-local)
  ready = true;
...
function animate(now) {
  ...
  if (ready) {
    if (scrollT < 0.03) {
      idleSpin += rawDt * 0.1;
      car.rotation.y = idleSpin * (1 - seg(scrollT, 0, 0.06));
    } else {
      car.rotation.y += (0 - car.rotation.y) * (1 - Math.exp(-10 * rawDt));
    }
    const hoodAmt = seg(scrollT, 0.72, 0.92);
    if (rig.hood) rig.hood.rotation.x = -hoodAmt * 0.9;
    engineLight.intensity = hoodAmt * 1.8;
  }
  ...
  fpsT += rawDt;
  if (fpsT >= 1) {
    window.__mustangFps = frames;
    window.__mustangCalls = renderer.info.render.calls;
    frames = 0;
    fpsT = 0;
    renderer.info.reset();
  }
